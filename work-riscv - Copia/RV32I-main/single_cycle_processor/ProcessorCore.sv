...
(
    ...
    // monociclo, uma instrucao "retira" a cada ciclo (exceto durante reset).
    ...
    logic [31:0]PC;
    logic [31:0]PC_if; // alias hierarquico esperado pelo top.sv (PrCore.PC_if)
    ...
    // PC_if e' apenas um espelho combinacional do PC atual, para o
    // testbench conseguir ler "PrCore.PC_if" (ver top.sv).
    assign PC_if = PC;

    // Sinais de "retire": neste core monociclo, a cada ciclo (fora reset)
    // uma instrucao termina fetch+decode+execute+writeback no mesmo pulso
    // de clock, entao ela "retira" nesse mesmo ciclo.
    assign retire_valid_o       = ~reset_i;
    assign retire_pc_o          = PC;
    assign retire_instruction_o = instruction;
    assign retire_reg_write_o   = reg_write_enable && (instruction[11:7] != 5'd0);
    assign retire_rd_o          = instruction[11:7];
    assign retire_rd_value_o    = reg_data;
